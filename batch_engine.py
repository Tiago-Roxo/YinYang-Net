...
def batch_trainer(epoch, model, train_loader, criterion, optimizer, att_dataset, att_model):
    model.train()
    ...
    for step, (imgs_all, imgs_face, gt_label, imgname, img_info) in enumerate(train_loader):
        
        batch_time = time.time()
        imgs_all, imgs_face, gt_label = imgs_all.cuda(), imgs_face.cuda(), gt_label.cuda()
        
        train_logits = model(imgs_all, imgs_face, img_info.float(), gt_label)

        train_logits = train_logits[:,att_model] # Gender
        gt_label = gt_label[:,att_dataset] # Gender
...
# @torch.no_grad()
def valid_trainer(model, valid_loader, criterion, att_dataset, att_model):
    model.eval()
    loss_meter = AverageMeter()
    ...
    with torch.no_grad():
        for step, (imgs_all, imgs_face, gt_label, imgname, img_info) in enumerate(tqdm(valid_loader)):
            ...
            imgs_all, imgs_face = imgs_all.cuda(), imgs_face.cuda()
            gt_label = gt_label.cuda()
            
            valid_logits = model(imgs_all, imgs_face, img_info.float())

            valid_logits = valid_logits[:,att_model] # Gender
            gt_label = gt_label[:,att_dataset] # Gender
